...
// TODO(yan): possibly we could port this function to `readLongsFully` function. It's more efficient.
...
void RleDecoderV2::unrolledUnpack56(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
        ...
        for (int i = 0; i < bufferNum; ++i) {
            ...
            buffer += 7;
            data[curIdx++] = static_cast<int64_t>((b0 << 48) | (b1 << 40) | (b2 << 32) | (b3 << 24) | (b4 << 16) |
                                                  (b5 << 8) | b6);
        }
        bufferStart = reinterpret_cast<const char*>(buffer);
        if (curIdx == offset + len) return;
        ...
        b6 = readByte();
        data[curIdx++] =
                static_cast<int64_t>((b0 << 48) | (b1 << 40) | (b2 << 32) | (b3 << 24) | (b4 << 16) | (b5 << 8) | b6);
    }
}
...
void RleDecoderV2::unrolledUnpack64(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
        ...
        for (int i = 0; i < bufferNum; ++i) {
            ...
            buffer += 8;
            data[curIdx++] = static_cast<int64_t>((b0 << 56) | (b1 << 48) | (b2 << 40) | (b3 << 32) | (b4 << 24) |
                                                  (b5 << 16) | (b6 << 8) | b7);
        }
        bufferStart = reinterpret_cast<const char*>(buffer);
        if (curIdx == offset + len) return;
...
RleDecoderV2::RleDecoderV2(std::unique_ptr<SeekableInputStream> input, bool _isSigned, MemoryPool& pool,
                           ReaderMetrics* _metrics, DataBuffer<char>* _sharedBufferPtr)
        : RleDecoder(_metrics),
...
void RleDecoderV2::adjustGapAndPatch(uint32_t patchBitSize, int64_t patchMask, int64_t* resGap, int64_t* resPatch,
                                     uint64_t* patchIdx) {
...
uint64_t RleDecoderV2::copyDataFromBuffer(int64_t* data, uint64_t offset, uint64_t numValues, const char* notNull) {
    uint64_t nRead = std::min(runLength - runRead, numValues);
    if (notNull) {
