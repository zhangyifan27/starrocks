...
Status KafkaDataConsumer::group_consume(StreamLoadContext* ctx, TimedBlockingQueue<RdKafka::Message*>* queue,
    ...
    while (true) {
        ...
        case RdKafka::ERR_OFFSET_OUT_OF_RANGE: {
            ...
            std::stringstream ss;
            ss << msg->errstr() << ", this is no data in partition: " << err_partition
               << " at offset: " << err_offset;
...
Status PulsarDataConsumer::assign_partition(StreamLoadContext* ctx,
...
Status PulsarDataConsumer::group_consume(TimedBlockingQueue<pulsar::Message*>* queue, int64_t max_running_time_ms) {
    ...
    while (true) {
        ...
        // consume 1 message at a time
        consumer_watch.start();
        pulsar::Result res = _p_reader.readNext(*(msg.get()), 1000 /* timeout, ms */);
        consumer_watch.stop();
        switch (res) {
