...
public class HiveScannerUtils {
    ...
    public static CombineFileSplit decodeStringToSplit(String encodedStr) {
        try {
            final byte[] bytes = BASE64_DECODER.decode(encodedStr.getBytes(UTF_8));
            CombineFileSplit split = new CombineFileSplit();
            ByteArrayInputStream baos = new ByteArrayInputStream(bytes);
            DataInputStream inputStream = new DataInputStream(baos);
