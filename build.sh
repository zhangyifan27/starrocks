...
if [ ${BUILD_FE} -eq 1 -o ${BUILD_SPARK_DPP} -eq 1 ]; then
    if [ ${BUILD_FE} -eq 1 ]; then
        ...
        cp -r -p ${STARROCKS_HOME}/conf/thiveFunction2SR.conf ${STARROCKS_OUTPUT}/fe/conf/
...
if [ ${BUILD_BE} -eq 1 ]; then
    ...
    cp -r -p ${STARROCKS_HOME}/be/output/conf/jmx_prometheus_export.yaml ${STARROCKS_OUTPUT}/be/conf/
