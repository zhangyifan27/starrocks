    ...
    while (i < fieldnum) {
      if (fieldtypes.containsKey(idx++)) {
        ...
        if (fieldtypes.get(idx - 1).type == ConstVar.FieldType_String
            || fieldtypes.get(idx - 1).type == ConstVar.FieldType_Binary) {
  ...
  private void serializeAll() throws IOException {
    ...
    for (int i = 0; i < this.fieldValues.size(); i++) {
      ...
      if (fv == null) {
        if (fixlenmode ||
            (oldformat &&
                ft.type != ConstVar.FieldType_String &&
                ft.type != ConstVar.FieldType_Binary)) {
...
//      LOG.info("roncen: params, oldformat=" + oldformat + ", VAR=" + VAR + 
//          ", fixlenmode=" + fixlenmode + ", contains=" + contains);
  ...
  public static class IFType implements Writable, IPersistable {
    ...
    public int len() {
      if (len == -1)
        ...
        case ConstVar.FieldType_Binary:
          len = 0;
          break;
        }
      return len;
  ...
  public static class IFValue implements Writable, IPersistable,
    ...
    // array, deep or low copy ??
    public IFValue(byte[] val, int idx) {
      this.type = new IFType(ConstVar.FieldType_Binary, idx);
      this.data = val;
    }
...
//        return ((String) data).length();
      if (this.type.type == ConstVar.FieldType_String) {
        return ((String) data).length();
      } else if (this.type.type == ConstVar.FieldType_Binary) {
    ...
    public void unpersistent(DataInput in) throws IOException {
      ...
      case ConstVar.FieldType_String:
        ...
        if (len < 0) {
          throw new IOException("column's length is more than 32K, please check it");
        }
...
//        LOG.info("roncen: read length=" + length);
        byte[] bs = new byte[length];
        in.readFully(bs);
//        for (byte bbb : bs) {
//          LOG.info("roncen: data=" + bbb);
//        }
        data = bs;
        break;
      }
    ...
    public void persistent(DataOutput bytes) throws IOException {
      ...
      case ConstVar.FieldType_Binary:
        ...
        if (bs.length > Integer.MAX_VALUE) {
          LOG.info("a record bigger than " + Integer.MAX_VALUE
              + " and has benn set to blank");
          length = 0;
        }
//        LOG.info("roncen: write length=" + length);
//        for (byte bbb : bs) {
//          LOG.info("roncen: data=" + bbb);
//        }
        bytes.writeInt(length);
        bytes.write(bs, 0, length);
        break;
      }
    }
    ...
    public int compareTo(IFValue ifv) {
      ...
      case ConstVar.FieldType_Binary: {
        byte[] leftData = (byte[]) this.data;
        byte[] rightData = (byte[]) ifv.data;
        int minLength = leftData.length < rightData.length ?
        ...
        return leftData.length > rightData.length ?
            1 : (leftData.length == rightData.length ? 0 : -1);
      }
      }
      return 0;
    }
    ...
    public IRecord.IFValue clone() {
      ...
      case ConstVar.FieldType_String:
        ...
        break;
      case ConstVar.FieldType_Binary:
        byte[] bs = (byte[]) this.data;
        cdata = new byte[bs.length];
        System.arraycopy(bs, 0, cdata, 0, bs.length);
      }
