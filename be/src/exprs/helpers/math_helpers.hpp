...
public:
    void add_field(const std::string& k, const std::string& v, std::string const& type) {
        _schema.insert(std::make_pair(k, std::make_tuple(false, v, type)));
    }
    void add_field(const std::string& v, std::string const& type) {
        _schema.insert(std::make_pair("$", std::make_tuple(false, v, type)));
    }
    void add_array_field(const std::string& k, const std::string& v, std::string const& type) {
        _schema.insert(std::make_pair(k, std::make_tuple(true, v, type)));
    }
    void add_array_field(const std::string& v, std::string const& type) {
        _schema.insert(std::make_pair("$", std::make_tuple(true, v, type)));
    }
...
private:
    void print(std::string& result, std::string const& now, std::string const& tab) {
        ...
        if (std::get<0>(range.first->second)) {
            result += "[";
        }
        result += "{";
        result += "\n";
        for (auto it = range.first; it != range.second; ++it) {
            result += tab + "  ";
            result += std::get<1>(it->second) + "(" + std::get<2>(it->second) + ")";
            print(result, std::get<1>(it->second), tab + "  ");
    ...
    std::multimap<std::string, std::tuple<bool, std::string, std::string>> _schema;
};
