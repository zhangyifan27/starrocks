...
public class ThiveRangePartitionPruner implements PartitionPruner {
    ...
    private List<Long> prune(RangeMap<PartitionKey, Long> rangeMap,
        ...
        if (null == filter) {
            ...
            try {
                result = Lists.newArrayList(
                        rangeMap.subRangeMap(Range.all()).asMapOfRanges().values());
            } catch (IllegalArgumentException e) {
                result = Lists.newArrayList();
            }
            return result;
        ...
        if (null == inPredicateLiterals || inPredicateLiterals.size() * complex > 100) {
            ...
            if (filter.lowerBound != null) {
                ...
                //Invalid data type for creating infinity: VARCHAR
            ...
            try {
                if (filter.lowerBound != null) {
                    result = Lists.newArrayList(
                            rangeMap.subRangeMap(Range.range(minKey, lowerType, maxKey, upperType)).asMapOfRanges()
                                    .values());
                } else {
                    result = Lists.newArrayList(
                            rangeMap.subRangeMap(Range.upTo(maxKey, upperType)).asMapOfRanges().values());
                }
            } catch (IllegalArgumentException e) {
                result = Lists.newArrayList();
            }
