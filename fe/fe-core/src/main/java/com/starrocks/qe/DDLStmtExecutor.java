...
import com.starrocks.datacache.DataCacheMetaManager.CacheDeleteTaskRequest;
...
public class DDLStmtExecutor {
    ...
    public static class StmtExecutorVisitor implements AstVisitor<ShowResultSet, ConnectContext> {
        ...
        public ShowResultSet visitDropTaskStmt(DropTaskStmt dropTaskStmt, ConnectContext context) {
            ...
            if (dropTaskStmt.isDataCacheJob()) {
                dropDataCacheJob(taskManager, task, context);
            } else {
        ...
        private void dropDataCacheJob(TaskManager taskManager, Task task, ConnectContext context) {
            String definition = task.getDefinition();
            if (Strings.isNullOrEmpty(definition)) {
                throw new SemanticException("Task " + task.getName() + " definition is empty");
            }

            int cachePartitionNum = Integer.parseInt(
                    task.getProperties().getOrDefault("cache_partition_num", "1"));
            String partitionUnit = task.getProperties().get("partition_unit");
            if (partitionUnit == null) {
                throw new SemanticException("Task " + task.getName() + " partitionUnit is empty");
            }
            ...
            for (int i = 1; i <= cachePartitionNum; i++) {
                String schedulePartition = DataCacheJobMgr.computeKthPreviousPartition(base, i, partitionUnit);
                LOG.info("DropDataCacheJob submit cache delete: task={}, partition={}, partitionUnit={}, base={}",
                        task.getName(), schedulePartition, partitionUnit, base);
                CacheDeleteTaskRequest cacheDelete = new CacheDeleteTaskRequest(
                        tableName.getCatalog(),
                        tableName.getDb(),
                        tableName.getTbl(),
                        schedulePartition,
                        CacheDeleteMode.NORMAL);
                metaManager.submitCacheDeleteTask(cacheDelete, context);
            }

            List<Long> tasksToDrop = Lists.newArrayList();
            LocalDateTime taskCreateTime =
                    LocalDateTime.ofInstant(Instant.ofEpochMilli(task.getCreateTime()), ZoneId.systemDefault());
            for (int i = 1; i <= cachePartitionNum; i++) {
                String manualCachePartition = DataCacheJobMgr.computeKthPreviousPartition(taskCreateTime, i, partitionUnit);
                LOG.info("DropDataCacheJob try drop partition task: baseCreateTime={}, partition={}, taskName={}",
                        taskCreateTime, manualCachePartition, task.getName() + "_" + manualCachePartition);
                Task partitionTask = taskManager.getTask(task.getName() + "_" + manualCachePartition);
                if (partitionTask != null) {
                    tasksToDrop.add(partitionTask.getId());
                }
            }

            tasksToDrop.add(task.getId());
            LOG.info("DropDataCacheJob drop tasks: taskIds={}, mainTaskId={}, taskName={}",
                    tasksToDrop, task.getId(), task.getName());
            taskManager.dropTasks(tasksToDrop, false);

            // set schedule task name to empty in data cache table meta
            metaManager.upsertTableMeta(tableName, null);
        }
