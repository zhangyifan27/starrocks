...
public class MergeFileUtil1 {
  public static final Log LOG = LogFactory.getLog(MergeFileUtil1.class);
  ...
  public static class MergeIFormatOutputFormat1<K, V> extends
      FileOutputFormat<LongWritable, IRecord> {

    public MergeIFormatOutputFormat1() {
    }
    ...
    public RecordWriter<LongWritable, IRecord> getRecordWriter(
      ...
      Path path = FileOutputFormat.getTaskOutputPath(job, name);
      String fileName = path.toString();
      final FormatDataFile fdf;
      try {
        fdf = new FormatDataFile(job);
        IHead ihead = new IHead();
        ihead.fromStr(job.get("ifdf.head.info"));
        Head head = new Head();
        head.setCompress(ihead.getCompress());
        head.setVar(ihead.getVar());
        HashMap<Integer, IRecord.IFType> fts = ihead.fieldMap().fieldtypes();
        final FieldMap fmap = new FieldMap();

        int i = 0, idx = 0;
        IRecord.IFType ft;
        while (i < fts.size()) {
          if ((ft = fts.get(idx++)) == null) {
            continue;
          }
          i++;
          fmap.addField(new Field(ft.type(), ft.len(), (short) ft.idx()));
        }

        head.setFieldMap(fmap);
        fdf.create(fileName, head);
        final short fieldNum = (short) fts.size();
        ...
        return new RecordWriter<LongWritable, IRecord>() {
          ...
          public void write(LongWritable key, IRecord value) throws IOException {
            rec.clear();
            rec.setFieldNum(fieldNum);
            try {

              for (int i = 0; i < fieldNum; i++) {
                IRecord.IFValue val = value.getByPos(i);
                FieldValue fv = new FieldValue();
                if (val == null) {
                  fv.setIdx((short) i);
                  fv.setType(fmap.getField((short) i).type());
                  fv.setLen(0);
                  rec.addValue(fv);
                  continue;
                }
                fv.setIdx((short) val.idx());
                fv.setType(val.type().type());
                fv.setLen(val.len());
                ByteBuffer bb = val.tobytes();
                byte[] v = new byte[bb.limit()];
                System.arraycopy(bb.array(), 0, v, 0, bb.limit());
                fv.setValue(v);
                rec.addValue(fv);
              }
              fdf.addRecord(rec);
  ...
  public static void run(String inputdir, String outputdir, Configuration conf)
    ...
    job.setInputFormat(FormatStorageInputFormat.class);
  ...
  public static void main(String[] args) throws IOException {
    if (args.length < 2) {
      System.out.println("please input the inputdir and outputdir");
      System.exit(0);
    }
