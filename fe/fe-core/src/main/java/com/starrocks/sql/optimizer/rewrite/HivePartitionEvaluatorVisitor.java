...
public class HivePartitionEvaluatorVisitor extends ScalarOperatorVisitor<Pair<Set<Long>, Boolean>, Void> {
    ...
    @Override
    public Pair<Set<Long>, Boolean> visit(ScalarOperator scalarOperator, Void context) {
        return Pair.create(null, true);
    }

    @Override
    public Pair<Set<Long>, Boolean> visitConstant(ConstantOperator literal, Void context) {
        return Pair.create(null, true);
    }

    @Override
    public Pair<Set<Long>, Boolean> visitBinaryPredicate(BinaryPredicateOperator binaryPredicate, Void context) {
        Set<Long> matches = null;
        Boolean existNoEval = false;
        matches = evalBinaryPredicate(binaryPredicate, context);
        return matches == null ? Pair.create(null, true) : Pair.create(matches, existNoEval);
    }

    private Set<Long> evalBinaryPredicate(BinaryPredicateOperator binaryPredicate, Void context) {
    ...
    @Override
    public Pair<Set<Long>, Boolean> visitCompoundPredicate(CompoundPredicateOperator compoundPredicate, Void context) {
        ...
        Pair<Set<Long>, Boolean> leftPair = compoundPredicate.getChild(0).accept(this, null);
        Set<Long> lefts = leftPair.first;

        Pair<Set<Long>, Boolean> rightPair = compoundPredicate.getChild(1).accept(this, null);
    ...
    @Override
    public Pair<Set<Long>, Boolean> visitInPredicate(InPredicateOperator inPredicate, Void context) {
        Set<Long> matches = null;
        Boolean existNoEval = false;
        matches = evalInPredicate(inPredicate, context);
        return matches == null ? Pair.create(null, true) : Pair.create(matches, existNoEval);
    }

    private Set<Long> evalInPredicate(InPredicateOperator inPredicate, Void context) {
        ...
        if (inPredicate.getChild(0) instanceof CallOperator && partitionValueMap != null) {
            // partitionValueMap need cast to target type
    ...
    @Override
    public Pair<Set<Long>, Boolean> visitIsNullPredicate(IsNullPredicateOperator isNullPredicate, Void context) {
        Set<Long> matches = null;
        Boolean existNoEval = false;
        matches = evalIsNullPredicate(isNullPredicate, context);
        return matches == null ? Pair.create(null, true) : Pair.create(matches, existNoEval);
    }

    private Set<Long> evalIsNullPredicate(IsNullPredicateOperator isNullPredicate, Void context) {
        Preconditions.checkNotNull(isNullPredicate);
        if (!isSinglePartitionColumnForIsNull(isNullPredicate)) {
            return null;
        }
    ...
    private boolean isSinglePartitionColumnForIsNull(ScalarOperator predicate) {
        List<ColumnRefOperator> columnRefOperatorList = Utils.extractColumnRef(predicate);
        if (columnRefOperatorList.size() == 1 && partitionColumnRefs.contains(columnRefOperatorList.get(0))) {
            // such int_part_column + 1 = 11 can't prune partition
            if (predicate.getChild(0).isColumnRef() ||
    ...
    private ConcurrentNavigableMap<LiteralExpr, Set<Long>> getCallPartitionValueMap(CallOperator callOperator,
        ...
        for (Map.Entry<LiteralExpr, Set<Long>> entry : partitionValueMap.entrySet()) {
            ...
            if (literalExpr.isPresent()) {
                Set<Long> partitions =
                        newPartitionValueMap.computeIfAbsent(literalExpr.isPresent() ? literalExpr.get() : key,
                                k -> Sets.newHashSet());
                partitions.addAll(entry.getValue());
            }
        }
    ...
    private Optional<LiteralExpr> computeCallPartitionValue(CallOperator callOperator, LiteralExpr literalExpr) {
