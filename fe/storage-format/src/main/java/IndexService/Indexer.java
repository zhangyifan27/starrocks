...
public class Indexer {
  ...
  public List<IRecord> get(String indexdir, String tblLocation, List<String> priParts, List<String> subParts,
                           List<IRecord.IFValue> values, int limit, String indexids, int fieldnum)
          throws Exception {
    return getRange(indexdir, tblLocation, priParts, subParts, values, values, limit, indexids, fieldnum);
  }
  ...
  public List<IRecord> getRange(String indexdir, String tblLocation, List<String> priParts, List<String> subParts,
                                List<IRecord.IFValue> startvalue, List<IRecord.IFValue> endvalue,
                                int limit, String indexids, int fieldnum) throws IOException {
    ...
    if (tblLocation == null) {
      throw new IOException("table location cannot be null");
    }
    ...
    if (priParts != null) {
      if (subParts != null) {
        for (String priPart : priParts) {
          for (String subPart : subParts) {
            if (limit > 0 && limit - result.size() <= 0)
              return result;
            result.addAll(getRange1(indexdir, tblLocation+"/"+priPart+"/"+subPart, valuestart1, valueend1, indexids,
                    limit - result.size(), fieldnum));
          }
        }
      }
        ...
        for (String priPart : priParts) {
          if (limit > 0 && limit - result.size() <= 0)
            break;
          result.addAll(getRange1(indexdir, tblLocation+"/"+priPart, valuestart1, valueend1, indexids,
                  limit - result.size(), fieldnum));
        }
      }
    }
      ...
      if (limit > 0 && limit - result.size() <= 0) {}
        ...
        result.addAll(getRange1(indexdir, tblLocation, valuestart1, valueend1, indexids,
                limit - result.size(), fieldnum));
    }
    return result;
  }
