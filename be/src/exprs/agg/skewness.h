...
using SkewnessAggregateState = MomentCalculator<long double, 3>;

template <LogicalType LT, bool is_sample, typename T = RunTimeCppType<LT>>
class SkewnessAggregateFunction
        : public AggregateFunctionBatchHelper<SkewnessAggregateState, SkewnessAggregateFunction<LT, is_sample, T>> {
public:
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        SkewnessAggregateState other(serialized_data);
    ...
    void finalize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
        ...
        double skewness = this->data(state).template skewness<is_sample>();
        column->append(skewness);
    }
